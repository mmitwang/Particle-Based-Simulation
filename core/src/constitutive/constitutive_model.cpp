...
Eigen::VectorXd ElasticModel::getParameters() const {
    ...
    params << young_modulus_, poisson_ratio_;
    return params;
}
...
Eigen::VectorXd MohrCoulombModel::getParameters() const {
    ...
    params << young_modulus_, poisson_ratio_, cohesion_, friction_angle_, dilation_angle_;
    return params;
}

// 检查模型是否处于屈服状态
bool MohrCoulombModel::isYielding(const StressStrainState& state) const {
    return computeYieldFunction(state) > 0.0;
}
...
Eigen::VectorXd ViscoElasticModel::getParameters() const {
    ...
    params << young_modulus_, poisson_ratio_, viscosity_, relaxation_time_;
    return params;
}
