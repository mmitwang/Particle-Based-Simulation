...
ContactDetection::ContactDetection(CollisionResponseModel model)
    : collision_model_(model),
      boundary_type_(BoundaryConditionType::RIGID),
...
void ContactDetection::initialize(
    ...
    const NeighborSearch& neighbor_searcher
) {
    neighbor_searcher_ = &neighbor_searcher;
    
    // 计算默认边界条件
    if (boundary_type_ == BoundaryConditionType::RIGID && boundary_params_.isZero()) {
        ...
        boundary_params_ << 
            bounds.min().x() - boundary_margin,
            bounds.max().x() + boundary_margin,
            bounds.min().y() - boundary_margin,
            bounds.max().y() + boundary_margin,
            bounds.min().z() - boundary_margin,
            bounds.max().z() + boundary_margin;
    }
}
...
void ContactDetection::setBoundaryCondition(const std::string& boundary_type, const Eigen::VectorXd& boundary_params) {
    if (boundary_type == "rigid") {
        ...
        boundary_type_ = BoundaryConditionType::PERIODIC;
    } else if (boundary_type == "damped") {
        boundary_type_ = BoundaryConditionType::DAMPED;
    } else {
        boundary_type_ = BoundaryConditionType::FREE;
    }
...
// 应用边界条件
void ContactDetection::applyBoundaryConditions(std::vector<Particle>& particles) {
    for (auto& particle : particles) {
        ...
        particle.applyForce(boundary_force);
    }
}
...
void ContactDetection::detectBoundaryContacts(
    ...
    if (boundary_type_ != BoundaryConditionType::RIGID) {
        return; // 目前只支持刚性边界
    }
...
// 应用边界条件
Eigen::Vector3d ContactDetection::applyBoundaryCondition(const Particle& particle) {
    // 目前只实现刚性边界条件
    if (boundary_type_ != BoundaryConditionType::RIGID) {
