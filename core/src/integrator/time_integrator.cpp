...
// 获取重力场
const Eigen::Vector3d& TimeIntegrator::getGravity() const {
    return gravity_;
}
...
// 计算粒子所受的所有力
Eigen::Vector3d TimeIntegrator::computeTotalForce(
    const std::vector<Particle>& particles,
    size_t particle_id,
    const ConstitutiveModel& constitutive_model,
    ContactDetection& contact_detection,
    ...
    // 简化实现，只考虑重力和接触力
    Eigen::Vector3d total_force = gravity_ * particles[particle_id].getMass();
...
bool TimeIntegrator::explicitEulerStep(
    ...
    // 5. 应用边界条件
    contact_detection.applyBoundaryConditions(particles);
